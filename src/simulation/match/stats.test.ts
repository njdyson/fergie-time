...
describe('StatsAccumulator', () => {
  ...
  describe('recordShot - actual kick events', () => {
    it('increments home shots', () => {
      const acc = new StatsAccumulator();
      acc.recordShot('home');
  ...
  describe('recordPass - actual kick events', () => {
    it('increments home passes', () => {
      const acc = new StatsAccumulator();
      acc.recordPass('home');
      const s = acc.getSnapshot();
    ...
    it('accumulates multiple passes', () => {
      const acc = new StatsAccumulator();
      acc.recordPass('home');
      acc.recordPass('home');
      const s = acc.getSnapshot();
  ...
  describe('recordIntent - no longer counts shots/passes', () => {
    it('SHOOT intent does not increment shots (counted via recordShot)', () => {
      const acc = new StatsAccumulator();
    ...
    it('PASS_FORWARD intent does not increment passes (counted via recordPass)', () => {
      const acc = new StatsAccumulator();
  ...
  describe('recordTackle', () => {
    ...
    it('PRESS intent does not count as tackle', () => {
      const acc = new StatsAccumulator();
  ...
  describe('getSnapshot immutability', () => {
    ...
    it('does not mutate previous snapshot when recording new events', () => {
      const acc = new StatsAccumulator();
      const s1 = acc.getSnapshot();
      acc.recordShot('home');
      const s2 = acc.getSnapshot();
...
describe('accumulateStats (functional API)', () => {
  ...
  it('accumulateStats preserves existing counts and updates possession', async () => {
    ...
    // Shots/passes are no longer counted from intents — only from actual kick events
    expect(result.shots).toEqual([0, 0]);
    expect(result.passes).toEqual([0, 0]);
